...
- [ ] `sitemap.xml` is generated by running the live site through
      https://www.xml-sitemaps.com/ and committing the tool's output
      exactly as downloaded, per the rubric's wording ("copied precisely
      from the sitemap generator tool output").
- [ ] Paste the generated `sitemap.xml` content here for the Word doc.
